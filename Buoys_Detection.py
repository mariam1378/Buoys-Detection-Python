...
while True:
    ...
    norm_img1 = cv2.normalize(frame, None, alpha=-0.25, beta=1.2, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
       
    # scale to uint8
    norm_img1 = np.clip(norm_img1, 0, 1)
    norm_img1 = (255*norm_img1).astype(np.uint8)

    hsv = cv2.cvtColor(norm_img1, cv2.COLOR_RGB2HSV)
    BGR = cv2.cvtColor(norm_img1, cv2.COLOR_BGR2HSV)
    ...
    # Upper_green = np.array([50,220,180])
    # mask0G      = cv2.inRange(hsv,Lower_green,Upper_green)
    # Lower_green = np.array([70,60,22])
    # Upper_green = np.array([90,255,255])
    # mask1G      = cv2.inRange(hsv,Lower_green,Upper_green)
    Lower_green  = np.array([37,0,0])
    Upper_green  = np.array([66,255,255])
    mask2G       = cv2.inRange(hsv,Lower_green,Upper_green)
    mask_Green   = mask2G#+(mask1G+mask0G)
    ...
    for i in range(len(contours)):
        ...
        if(area>100):
            cv2.rectangle(norm_img1, (int(boundRect[i][0]), int(boundRect[i][1])), (int(boundRect[i][0]+boundRect[i][2]), int(boundRect[i][1]+boundRect[i][3])), (0, 255, 0), 2)
            cv2.putText(norm_img1, 'GREEN BUOY - Safe', (int(boundRect[i][0]), int(boundRect[i][1])), 1, 1, color=(0, 255, 0),thickness=2)
    ########################################################################################################
    ########################################################################################################
    #Red Buoy Contours
    ...
    for i in range(len(contours)):
        area = cv2.contourArea(contours[i])
        if(area>100):
            cv2.rectangle(norm_img1, (int(boundRect[i][0]), int(boundRect[i][1])), (int(boundRect[i][0]+boundRect[i][2]), 
    ...
    for i in range(len(contours)):
        area = cv2.contourArea(contours[i])
        if(area>50):
            cv2.rectangle(norm_img1, (int(boundRect[i][0]), int(boundRect[i][1])), (int(boundRect[i][0]+boundRect[i][2]), int(boundRect[i][1]+boundRect[i][3])), (0,255,255), 2)
            cv2.putText(norm_img1, 'YELLOW BUOY - Working', (int(boundRect[i][0]), int(boundRect[i][1])), 1, 1, color=(0,255,255),thickness=2)
    ########################################################################################################

    result_Green=cv2.bitwise_and(norm_img1,norm_img1,mask=mask_Green)
    result_Red=cv2.bitwise_and(norm_img1,norm_img1,mask=mask_Red)
    result_Yellow=cv2.bitwise_and(norm_img1,norm_img1,mask=mask_Yellow)
    norm_img1_width = int(capture.get(3))
    norm_img1_height = int(capture.get(4))

    out = cv2.VideoWriter('Buoy1_Enhancement.avi',cv2.VideoWriter_fourcc('M','J','P','G'), 10, (norm_img1_width,norm_img1_height))
     #--> out
     #-->inside
    #cv2.waitKey(0)
    if isTrue:    
        cv2.imshow('Video', norm_img1)
        out.write(norm_img1)
